-- Migraci├│n para crear la tabla maestra rutas e integrar id_ruta en clientes
...
COMMENT ON TABLE public.rutas IS 'Tabla maestra de rutas de despacho y distribuci├│n';
COMMENT ON COLUMN public.rutas.id_ruta IS 'Identificador ├║nico de la ruta (UUID)';
COMMENT ON COLUMN public.rutas.nombre_ruta IS 'Nombre identificador de la ruta (no nulo)';
COMMENT ON COLUMN public.rutas.descripcion_ruta IS 'Descripci├│n o detalles adicionales de la ruta (acepta nulo)';

-- 2. Habilitar RLS en rutas y definir pol├¡ticas de acceso
